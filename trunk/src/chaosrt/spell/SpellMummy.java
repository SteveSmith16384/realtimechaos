...
import chaosrt.server.ServerMummy;
import chaosrt.server.ServerPlayer;

public class SpellMummy extends Spell {

  public SpellMummy() {
    super(Spell.MUMMY, 1000, "Mummy", Spell.CREATURE_RANGE);
  }
  ...
  public boolean cast(ServerPlayer player, int x, int y, boolean illusion) throws IOException {
    ...
    if (dist <= radius) {
      ServerMummy mummy = new ServerMummy(x, y, player.side, illusion);
      mummy.addToMap();
    if (illusion == false) {
