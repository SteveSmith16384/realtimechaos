...
import chaosrt.server.ServerDragon;
import chaosrt.server.ServerPlayer;

public class SpellDragon extends Spell {

  public SpellDragon() {
    super(Spell.DRAGON, 4200, "Dragon", Spell.CREATURE_RANGE);
  }
  ...
  public boolean cast(ServerPlayer player, int x, int y, boolean illusion) throws IOException {
    ...
    if (dist <= radius) {
      ServerDragon dragon = new ServerDragon(x, y, player.side, illusion);
      dragon.addToMap();
      if (illusion == false) {
